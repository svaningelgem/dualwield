		...
		if (damage > 0.0F || enchantmentLevel > 0.0F) {
			...
			if (flag5) {
				...
				if (shouldSweep) {
					...
					entityPlayer.world.a(null, entityPlayer.locX, entityPlayer.locY, entityPlayer.locZ, SoundEffects.eg, entityPlayer.bC(), 1.0F, 1.0F);
					entityPlayer.cM();
				}
				...
				if (!entityPlayer.world.isClientSide && nmsEntity instanceof EntityHuman) {
					...
					ItemStack humanItemStack = entityHuman.cx() ? entityHuman.cA() : null;
